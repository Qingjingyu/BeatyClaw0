#!/usr/bin/env node
/**
 * C4 Communication Bridge Interface for zylos-slack
 *
 * Sends messages to Slack via the internal HTTP API of the main process.
 *
 * Usage:
 ...
 *
 * Endpoint format:
 *   channelId|type:im|msg:ts
 *   channelId|type:channel|msg:ts
 */
...
const msgTs = parsed.msg || '';
const chatType = parsed.type || 'im';

if (message.trim() === '[SKIP]') {
  if (!msgTs) process.exit(0);
}

if (!config.enabled) {
  console.error('[slack] Channel disabled');
  process.exit(1);
}
...
async function internalSend(target, content, skip = false) {
...
async function send() {
  try {
    ...
    await internalSend(targetChannel, skip ? '' : message, skip);
