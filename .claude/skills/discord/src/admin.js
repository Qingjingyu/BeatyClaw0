...
const commands = {
  ...
  'set-token': (token, appId) => {
    if (!token) {
      console.error('Usage: admin.js set-token <bot_token> [app_id]');
      process.exit(1);
    }
    ...
    console.log('Restart service: pm2 restart zylos-discord');
  },
  ...
  'list-groups': () => {
    ...
    for (const [id, cfg] of entries) {
      ...
      console.log(`  ${id} - ${cfg.name || 'unnamed'}${allowFrom}`);
    }
  },

  'add-group': (groupId, name) => {
    if (!groupId || !name) {
      console.error('Usage: admin.js add-group <channelId|guildId> <name>');
      process.exit(1);
    }
    const config = loadConfig();
    ...
    saveConfigOrExit(config);
    console.log(`Group added: ${groupId} (${name})`);
    console.log('Restart service: pm2 restart zylos-discord');
  },

  'remove-group': (groupId) => {
    if (!groupId) {
      console.error('Usage: admin.js remove-group <channelId|guildId>');
      process.exit(1);
    }
    const config = loadConfig();
    if (config.groups?.[groupId]) {
      const name = config.groups[groupId].name;
      delete config.groups[groupId];
      saveConfigOrExit(config);
      console.log(`Group removed: ${groupId} (${name})`);
    } else {
  ...
  'help': () => {
    console.log(`zylos-discord admin CLI

Commands:
  show                              Show full config
  show-owner                        Show current owner
  set-token <token> [appId]         Save bot token (and optional app ID) to .env
  ...
  add-group <id> <name>             Add a group (channelId or guildId)
  remove-group <id>                 Remove a group
