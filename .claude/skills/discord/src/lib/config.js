...
export const DATA_DIR = path.join(HOME, 'zylos/components/discord');
...
export function loadConfig() {
  try {
    ...
    console.error(`[discord] Config load failed: ${err.message}`);
    config = { ...DEFAULT_CONFIG };
  }
  return config;
}
...
export function saveConfig(newConfig) {
  ...
  try {
    ...
    console.error(`[discord] Config save failed: ${err.message}`);
...
export function watchConfig(onChange) {
  ...
  if (fs.existsSync(configDir)) {
    ...
    configWatcher.on('error', (err) => {
      console.warn(`[discord] Config watch error: ${err.message}`);
      if (configReloadTimer) { clearTimeout(configReloadTimer); configReloadTimer = null; }
...
export function saveCredentialsToEnv({ bot_token, app_id }) {
  ...
  setLine('DISCORD_BOT_TOKEN', token);
  if (appId) setLine('DISCORD_APP_ID', appId);
  ...
  if (appId) process.env.DISCORD_APP_ID = appId;
