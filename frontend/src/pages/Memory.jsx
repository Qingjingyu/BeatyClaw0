...
function TreeNode({ node, selectedPath, onSelect, depth = 0 }) {
  ...
  return (
    ...
    >
      ...
      <span className="ml-auto text-xs text-slate-600 flex-shrink-0">{formatSize(node.size)}</span>
    </button>
  )
}
  ...
  return (
    <div className="p-4 lg:p-6 h-[calc(100vh-4rem)]">
      <div className="flex flex-col lg:flex-row gap-4 h-full">
        ...
        <div className="w-full lg:w-72 flex-shrink-0 rounded-xl border border-slate-800/50 bg-slate-900/50 flex flex-col overflow-hidden">
        ...
        <div className="flex-1 rounded-xl border border-slate-800/50 bg-slate-900/50 flex flex-col overflow-hidden min-h-0">
          {/* File header */}
          <div className="px-4 py-3 border-b border-slate-800/50 flex items-center justify-between gap-4 flex-shrink-0">
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              {saveMsg && (
                <span className={cn(
                  'text-xs',
                  saveMsg === '已保存' ? 'text-emerald-400' : 'text-red-400'
                )}>
                  {saveMsg}
                </span>
              )}
          ...
          <div className="flex-1 overflow-auto min-h-0">
            {fileLoading ? (
              ...
              <textarea
                ...
                className="w-full h-full p-4 bg-transparent text-slate-200 text-sm font-mono leading-relaxed resize-none focus:outline-none"
                spellCheck={false}
              />
            ) : (
              <pre className="p-4 text-sm text-slate-200 font-mono leading-relaxed whitespace-pre-wrap break-words">
                {fileContent}
              </pre>
            )}
          </div>
